package com.example.BE_PBL6_FastOrderSystem.utils.constants;

public class ZaloPayConstant {
	public static final String REFUND_URL = "https://sandbox.zalopay.com.vn/v001/tpe/refund";
	public static String APP_ID = "2553";
	public static String KEY1 = "PcY4iZIKFCIdgZvA6ueMcMHHUbRLYjPL";
	public static String KEY2 = "kLtgPl8HHhfvMuDHPwKfgfsY4Ydm9eIz";
	public static String CREATE_ORDER_URL = "https://sandbox.zalopay.com.vn/v001/tpe/createorder";
	public static String GET_STATUS_PAY_URL = "https://sandbox.zalopay.com.vn/v001/tpe/getstatusbyapptransid";
	public static String REDIRECT_URL = "https://ambitious-bush-01a78f310.4.azurestaticapps.net";
}